...
状态：Active execution plan（WP-00 至 WP-10 completed；WP-11 的本地实现已完成，
但 exact clean-commit RC 因未获 commit 授权而 blocked；WP-07B、WP-12、WP-13 尚未执行）
...
- 不删除测试数量来降低门槛；等价重构必须保留一一映射的验证证据；
...
2. 选择依赖已满足的第一个未完成工作包；
3. 先写或确认失败测试，再修改 owner implementation；
...
| WP-01 Threat model 与版本决策 | completed | Evidence Ledger `WP-01`；独立审计无 P0/P1；完整 authority v2 profile 仍未激活 |
| WP-02 StateStore v2 与 historical finality | completed | Evidence Ledger `WP-02`；Draft Store slice 已独立审计，完整 authority profile 仍 inactive/unsupported |
| WP-03 Scope-bound issuer capability | completed | Evidence Ledger `WP-03`；public Draft Authority Session v2 与双 Store Conformance 已闭环，完整 authority profile 仍 inactive/unsupported |
...
| WP-11 RC dry-run、workflow 与供应链 | blocked | workflow、SBOM、reproducibility、main/tag/repository/immutable-release proposals、external wheel/sdist consumer 与本地 policy shard 已完成；exact RC 必须绑定 clean candidate commit，当前工作树未提交且未获 commit 授权，fail-closed 拒绝构建 |
| WP-12 独立 reference runtime | planned | 等待 WP-11 exact candidate wheel/commit 与独立 runtime 仓授权；不得在 protocol-core 内实现 |
...
目标：在本地和 CI 完成可验证的 Draft promotion candidate、release workflow、exact candidate artifacts
和完整 dry-run。真实 merge、tag、GitHub Release、Stable lifecycle promotion 和 PyPI 属于 WP-13 的显式
远程授权 Goal。
...
- 每个 active profile 声明或实际可产生的 Governance-issued terminal outcome 均可 delivery；
...
Core 实现时逐行追加，不覆盖旧证据。Runtime 只追加 compatibility summary；完整 R0-R9A evidence 与
