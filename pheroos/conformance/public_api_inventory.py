...
PUBLIC_API_INVENTORY_PATH = Path("pheroos/conformance/abi/public-python-api-v1.json")
PUBLIC_PACKAGES = (
    "pheroos.protocol",
    "pheroos.governance",
    "pheroos.kernel",
...
def _collect_sequence_differences(
    expected: list[Any],
    observed: list[Any],
    path: str,
    differences: list[str],
...
def _collect_named_differences(
...
def _named_exports(items: list[Any]) -> dict[str, Any] | None:
    if not all(
        isinstance(item, dict) and isinstance(item.get("name"), str) for item in items
    ):
        return None
...
def _module_binding_origins(
    ...
    if static_public_api is not None:
        return _static_binding_origins(package_name, static_public_api)
    source_path = getattr(module, "__file__", None)
    if source_path is None:
        return {}
    return _source_binding_origins(package_name, Path(source_path))


def _static_binding_origins(
    package_name: str,
    static_public_api: Any,
) -> dict[str, tuple[str, str, bool]]:
    origins: dict[str, tuple[str, str, bool]] = {}
    for name, target in static_public_api.items():
        if not isinstance(name, str) or not _valid_static_public_target(target):
            raise ValueError(
...
def _valid_static_public_target(target: object) -> TypeGuard[tuple[str, str]]:
    return (
...
def _record_binding_origin_node(
    node: ast.stmt,
    *,
    package_name: str,
    origins: dict[str, tuple[str, str, bool]],
    ...
    if isinstance(node, ast.ImportFrom):
        _record_import_from_origins(node, package_name, origins)
    elif isinstance(node, ast.Import):
        _record_imported_modules(node, imported_modules)
    elif isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
        origins[node.name] = (package_name, node.name, True)
    elif isinstance(node, (ast.Assign, ast.AnnAssign)):
        _record_assigned_origins(node, package_name, origins, imported_modules)


def _record_import_from_origins(
    node: ast.ImportFrom,
    package_name: str,
    origins: dict[str, tuple[str, str, bool]],
) -> None:
...
def _property_shape(name: str, descriptor: property) -> dict[str, Any]:
    ...
    return {
        ...
        and (inspect.iscoroutinefunction(getter) or inspect.isasyncgenfunction(getter)),
        ...
        and (inspect.iscoroutinefunction(setter) or inspect.isasyncgenfunction(setter)),
...
def _value_shape(value: Any, active: set[int] | None = None) -> Any:
    handled, shaped = _primitive_value_shape(value, active)
    if handled:
        return shaped
    handled, shaped = _special_value_shape(value)
    if handled:
        return shaped
    return _recursive_value_shape(value, active)


def _primitive_value_shape(
    value: Any,
    active: set[int] | None,
) -> tuple[bool, Any]:
    ...
    if isinstance(value, bytes):
        return True, {"hex": value.hex(), "kind": "bytes"}
...
def _special_value_shape(value: Any) -> tuple[bool, Any]:
    if value is Ellipsis:
        return True, {"kind": "ellipsis"}
    if value is NotImplemented:
        return True, {"kind": "not-implemented"}
    if isinstance(value, PurePath):
        return True, {"kind": "path", "value": _stable_path(value)}
    ...
    if inspect.isclass(value) or inspect.isroutine(value):
        return True, {"identity": _identity(value), "kind": "object-reference"}
    return False, None


def _recursive_value_shape(value: Any, active: set[int] | None) -> Any:
    if active is None:
        active = set()
    object_id = id(value)
    ...
    try:
        return _container_value_shape(value, active)
    finally:
        active.remove(object_id)


def _container_value_shape(value: Any, active: set[int]) -> Any:
    ...
    raise TypeError(f"unsupported public ABI value type: {_type_identity(type(value))}")
