"""Exact upstream source proof and preparation for Commit Permission v2."""
...
@final
class VerifiedCommitPermissionSourceV2(_VerifiedGateSourceBaseV2):
    """Non-portable proof for one exact action-permission request."""

    def __new__(
        cls, *_args: object, **_kwargs: object
    ) -> VerifiedCommitPermissionSourceV2:
        raise TypeError(
            "VerifiedCommitPermissionSourceV2 cannot be constructed directly"
        )

    def __init_subclass__(cls, **_kwargs: object) -> NoReturn:
        raise TypeError("VerifiedCommitPermissionSourceV2 is final")

    def __repr__(self) -> str:
        return "<VerifiedCommitPermissionSourceV2 redacted>"


def prepare_commit_permission_issue_v2(
    *,
    domain_root: str,
    scope_ref: str,
    ...
    parent_snapshot: CommitPermissionSnapshotV2 | None = None,
) -> tuple[CommitPermissionRequestV2, VerifiedCommitPermissionSourceV2]:
    """Prepare permission bound to the exact declared candidate set and claims."""
    ...
    parent_revision, parent_transition, parent_root = _permission_parent(
        parent_snapshot
    )
    stream_ref = commit_permission_stream_ref_v2(
        scope_ref, context.protocol_ref, run_ref, target_ref
    )
    transition_id = commit_permission_transition_id_v2(stream_ref, permission_ref)
    ...
    candidates = context.candidate_refs
    claims = tuple(claim_roots)
    snapshot = CommitPermissionSnapshotV2(
        domain_root=domain_root,
        scope_ref=scope_ref,
        manifest_root=context.manifest_root,
        commit_policy_root=context.commit_policy_root,
        policy_root=commit_permission_policy_root_v2(
            manifest_root=context.manifest_root,
            commit_policy_root=context.commit_policy_root,
            protocol_ref=context.protocol_ref,
            target_ref=target_ref,
        ...
        candidate_set_root=commit_gate_candidate_set_root_v2(candidates),
        claim_roots=claims,
        claims_root=commit_gate_claims_root_v2(claims),
    ...
    source = _issue_gate_source_v2(
        VerifiedCommitPermissionSourceV2,
        kind="permission",
        request=CommitPermissionRequestV2.from_dict(request.to_dict()),
    ...
    return request, cast(VerifiedCommitPermissionSourceV2, source)


def verify_commit_permission_request_source_v2(
    request: CommitPermissionRequestV2,
    *,
    source: object,
    committed_parent_snapshot: CommitPermissionSnapshotV2 | None,
) -> tuple[str, tuple[GovernanceReadPreconditionV2, ...]]:
    """Recompute every permission source commitment from canonical inputs."""

    if type(request) is not CommitPermissionRequestV2:
        raise TypeError("commit permission source requires exact request v2")
    stored, manifest, material, preconditions = _verified_gate_source_fields_v2(
        source,
        expected_type=VerifiedCommitPermissionSourceV2,
        expected_kind="permission",
        expected_request_type=CommitPermissionRequestV2,
    )
    detached = CommitPermissionRequestV2.from_dict(
        cast(CommitPermissionRequestV2, stored).to_dict()
    )
    if detached.to_dict() != request.to_dict():
        raise ValueError("commit permission source request is mismatched")
    ...
    if (
        ...
        or context.candidate_refs != snapshot.candidate_refs
    ):
        raise ValueError("commit permission source manifest context is mismatched")
    if committed_parent_snapshot is None:
        if snapshot.revision != 1:
            raise ValueError("commit permission source parent is missing")
    else:
    ...
    if (
        ...
        raise ValueError("commit permission source proof is mismatched")
    return expected_source_root, expected_preconditions


def _permission_parent(
    parent: CommitPermissionSnapshotV2 | None,
) -> tuple[int, str, str]:
    if parent is None:
        return (
            0,
    ...
    if type(parent) is not CommitPermissionSnapshotV2:
        raise TypeError("commit permission parent must be exact snapshot v2")
    detached = CommitPermissionSnapshotV2.from_dict(parent.to_dict())
