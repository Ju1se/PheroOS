...
def _capped_pheromone_score_contributions(
    *,
    ...
    sources: dict[str, set[str]] = {
        candidate.id: set() for candidate in candidate_set.candidates
    }
    ...
    for trail in ordered:
        if current_step is not None and is_expired_with_policy(
            trail, policy, current_step
        ):
            continue
        ...
        if (
            trail.kind == "novelty"
            and policy.exploration_enabled
...
def score_pheromone_trails_result(
    *,
    ...
    competitive_response = (
        policy.response_model == "competitive" or policy.competition_mode == "normalize"
    )
    ...
    for trail, candidate_id, _, delta, category in contributions:
        ...
        add_dimension_breakdown(
            subject_breakdown, candidate_id, subject_dimension, delta
        )
        if trail.kind == "positive":
            ...
            "cautionary",
            "alarm",
        } and pheromone_kind_can_suppress_positive(trail.kind, policy):
    ...
    for candidate_id, cautionary in suppressing_support.items():
        if cautionary > 0 and cautionary >= policy.cautionary_override_threshold:
            ...
            for subject_type, positive_delta in positive_subject_support[
                candidate_id
            ].items():
                add_dimension_breakdown(
                    subject_breakdown,
    ...
    if policy.response_exploration_floor > 0:
        response_floor = min(
            float(policy.max_strength), float(policy.response_exploration_floor)
        )
    ...
    if policy.exploration_enabled and policy.exploration_floor > 0:
        for candidate in candidate_set.candidates:
            ...
            add_breakdown(
                breakdown, candidate_id, "pheromone_novelty", policy.exploration_floor
            )
            add_dimension_breakdown(
                kind_breakdown,
                candidate_id,
    ...
    if competitive_response:
        ...
        for candidate_id, offset in normalization_offsets.items():
            add_dimension_breakdown(
                kind_breakdown, candidate_id, "normalization", offset
            )
            add_dimension_breakdown(
                subject_breakdown, candidate_id, "candidate", offset
            )
    ...
    for candidate_id in scores:
        ...
        if abs(math.fsum(kind_breakdown[candidate_id].values()) - reconstructed) > 1e-9:
            raise GovernanceError(
                "pheromone kind breakdown does not reconstruct candidate score"
            )
        if (
            abs(math.fsum(subject_breakdown[candidate_id].values()) - reconstructed)
            > 1e-9
        ):
            raise GovernanceError(
                "pheromone subject breakdown does not reconstruct candidate score"
            )
...
def raw_pheromone_delta(
    ...
    weight = (
        profile.weight
        if profile is not None
        else legacy_pheromone_weight(trail.kind, policy)
    )
...
def apply_pheromone_response(
    delta: float, trail: PheromoneTrail, policy: PheromonePolicy
) -> float:
...
def add_dimension_breakdown(
    ...
    if not math.isfinite(updated):
        raise GovernanceError(
            f"pheromone dimension breakdown must remain finite: {dimension}"
        )
...
def observe_pheromone_exploration(
    *,
    ...
    for _, trail in sorted(
        ...
        is_stale_route = pheromone_subject_type(trail) == "route" and (
            trail.kind == "stale" or expired
        )
        novelty_pressure = 0.0
        if trail.kind == "novelty" and not expired:
            elapsed = current_step - trail.updated_at_step
            novelty_pressure = float(trail.strength) * (
                (1.0 - policy.novelty_decay_rate) ** elapsed
            )
            novelty_pressure = min(
                float(policy.max_strength), max(0.0, novelty_pressure)
            )
        reopen_eligible = is_stale_route and float(trail.strength) <= float(
            policy.stale_route_reopen_threshold
        )
        if novelty_pressure <= 0 and not reopen_eligible:
            continue
        reason = (
            "stale_route_reopen_eligible"
            if reopen_eligible
            else "novelty_pressure_observed"
        )
        observations.append(
            PheromoneExplorationObservation(
                target=trail.target,
...
for _compat_function in (
    ...
    _capped_pheromone_score_contributions,
    score_pheromone_trails,
    score_pheromone_trails_with_breakdown,
    score_pheromone_trails_result,
    pheromone_kind_can_suppress_positive,
    raw_pheromone_delta,
    kind_breakdown_category,
    subject_breakdown_category,
    apply_pheromone_response,
    normalize_pheromone_scores,
    empty_score_breakdown,
    add_breakdown,
    add_dimension_breakdown,
    observe_pheromone_exploration,
    cap_source_contribution,
):
