"""Shared, stateless Risk vocabulary and policy invariants.

This leaf owns no issuance, registry, cursor, replay state, or Store access. It
is shared by the legacy Draft facade and durable Risk v2 so the latter does not
depend on a process-local authority owner merely to interpret protocol policy.
"""
...
class _NormalizedRiskBindings(TypedDict):
    profile: str
...
def _normalized_bindings(
    *,
    ...
    return {
        ...
        "protocol_id": require_commit_text(protocol_id, f"{field_name} protocol_id"),
...
def _validate_risk_table(policy: CollectiveCommitPolicy) -> None:
    diagnostics = validate_risk_bands(
        policy, path="collective_commit_policy.risk_bands"
    )
    if diagnostics:
...
# Preserve the long-standing public enum owner independently of import order.
RiskBand.__module__ = "pheroos.governance.risk"


__all__ = ["RiskBand", "risk_policy_root"]
