...
class HybridCommitStep:
    ...
    Only ``commit_assessment`` supplies commit status, leader, metrics and
    truth roots.  Attention is carried beside that projection for runtime
    exploration; its roots never enter ``commit_truth_root`` or
    ``commit_metrics_root`` and therefore cannot affect a certificate.
    """
...
def bind_hybrid_commit_channels(
    *,
    ...
    """Bind independent attention and commit channels without recomputation.

    This function intentionally does not call ``score_candidates`` or the
    legacy collective decision function.  It accepts only an attention-only
    Hybrid step and an already governance-issued Optimal Commit assessment.
    """
...
def evaluate_hybrid_commit_step(*, request: object):
    """Run the total Hybrid Commit evaluation entry.

    The former three-channel composition operation is intentionally exposed as
    :func:`bind_hybrid_commit_channels`.  This public evaluator accepts the
    strict total-function request and returns a HybridCommitEvaluation; it does
    not retain an overload that could bypass liveness/finality/output gates.
    """

    return evaluate_hybrid_commit_evaluation(request)
...
def hybrid_commit_truth_projection(step: HybridCommitStep) -> dict[str, Any]:
    """Return the certificate-facing projection, excluding all attention data."""
...
def hybrid_attention_projection(step: HybridCommitStep) -> dict[str, Any]:
    """Return the exploration-facing projection, excluding commit truth fields."""
...
def _validate_hybrid_commit_step_shape(step: HybridCommitStep) -> None:
    ...
    if step.unique_leader:
        if not isinstance(step.leader_candidate_id, str) or not step.leader_candidate_id:
