...
NON_SCORING_ACTIONS = frozenset(
    {"request_scouting", "fallback_pressure", "confirm_trace_coverage", "propose_pheromone"}
)
...
class LayerCoordinationState:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "allocated_weights", MappingProxyType(dict(self.allocated_weights)))
        ...
        object.__setattr__(self, "action_effects", MappingProxyType(dict(self.action_effects)))
        object.__setattr__(self, "pheromone_proposal_trace_ids", tuple(self.pheromone_proposal_trace_ids))
...
def layer_coordination_policy_from_collective(policy: object) -> LayerCoordinationPolicy:
    return LayerCoordinationPolicy(
        enabled=policy.layer_coordination_enabled,
        layer_weight_bounds=dict(policy.layer_weight_bounds),
        default_layer_weights=dict(policy.layer_default_weights),
        confidence_thresholds=dict(policy.layer_confidence_thresholds),
        conflict_threshold=policy.layer_conflict_threshold,
        emergency_override_threshold=policy.layer_emergency_override_threshold,
        min_layer_provenance=policy.layer_min_provenance,
        fallback_on_unresolved_conflict=policy.layer_fallback_on_unresolved_conflict,
        policy_adjustment_bounds=dict(policy.policy_adjustment_bounds),
    )
...
def validate_layer_coordination_policy(policy: LayerCoordinationPolicy) -> None:
    ...
    conflict_threshold = _finite_number(policy.conflict_threshold, "layer conflict_threshold")
    ...
    if isinstance(policy.min_layer_provenance, bool) or not isinstance(policy.min_layer_provenance, int):
    ...
    if policy.enabled and not policy.fallback_on_unresolved_conflict:
        raise GovernanceError("enabled layer coordination must fall back on unresolved conflict")
    ...
    for mapping_name, mapping in (
        ...
        for layer_id, value in mapping.items():
            if layer_id not in SUPPORTED_LAYER_IDS:
                raise GovernanceError(f"unsupported layer id in {mapping_name}: {layer_id}")
            number = _finite_number(value, f"layer {mapping_name} {layer_id}")
            if number < 0:
                raise GovernanceError(f"layer {mapping_name} must be non-negative: {layer_id}")
            if mapping_name == "confidence threshold" and number > 1:
                raise GovernanceError(f"layer confidence threshold must not exceed 1: {layer_id}")
    for layer_id, bounds in policy.layer_weight_bounds.items():
        ...
        if not isinstance(bounds, (tuple, list)) or len(bounds) != 2:
            raise GovernanceError(f"layer weight bounds must contain two values: {layer_id}")
        ...
        if default is not None and not lower <= default <= upper:
            raise GovernanceError(f"layer default weight is outside declared bounds: {layer_id}")
...
def validate_layer_proposal(
    ...
    *,
    ...
    if not isinstance(proposal.target, str) or proposal.target != target:
        raise GovernanceError(f"layer proposal targets {proposal.target}, not active target {target}")
    if proposal.action not in SUPPORTED_LAYER_ACTIONS and not is_extension_layer_action(proposal.action):
    ...
    for field_name in ("support", "risk", "proposed_strength"):
        value = _finite_number(getattr(proposal, field_name), f"layer proposal {field_name}")
        if not 0 <= value <= MAX_LAYER_MAGNITUDE:
            raise GovernanceError(f"layer proposal {field_name} is outside absolute bounds")
    if proposal.proposed_pheromone_kind:
        if (
            ...
            raise GovernanceError(f"unsupported proposed pheromone kind: {proposal.proposed_pheromone_kind}")
        if not proposal.candidate_id:
            raise GovernanceError("proposed pheromone must be bound to a declared candidate")
        ...
        if not isinstance(subject_type, str) or not isinstance(subject_id, str) or not subject_id:
        ...
        if subject_type == "candidate" and subject_id != proposal.candidate_id:
            raise GovernanceError("proposed candidate pheromone subject must match candidate_id")
        if proposal.proposed_pheromone_kind in {"alarm", "cautionary"} and proposal.layer_id != "reactive":
            raise GovernanceError("only the reactive layer may propose emergency pheromone pressure")
    if proposal.action == "propose_pheromone":
        if not proposal.proposed_pheromone_kind:
            raise GovernanceError("propose_pheromone action requires proposed_pheromone_kind")
        if proposal.proposed_strength <= 0:
            raise GovernanceError("propose_pheromone action requires positive proposed_strength")
    ...
    if proposal.action in EMERGENCY_ACTIONS and proposal.proposed_pheromone_kind:
        if proposal.proposed_pheromone_kind != proposal.action:
            raise GovernanceError("reactive emergency action and proposed pheromone kind must match")
    if proposal.action == "resolve_conflict" and proposal.layer_id != "metacognitive":
        raise GovernanceError("only the metacognitive layer may propose conflict resolution")
    if proposal.action == "confirm_trace_coverage" and proposal.layer_id != "metacognitive":
        raise GovernanceError("only the metacognitive layer may confirm trace coverage")
    if proposal.action not in {"request_scouting", "fallback_pressure"} and not is_extension_layer_action(proposal.action):
    ...
    *,
    ...
    for proposal in items:
        if proposal.trace_event_id in trace_ids:
            raise GovernanceError(f"duplicate layer proposal trace_event_id: {proposal.trace_event_id}")
...
def layer_action_effect(
    ...
    raise GovernanceError(f"layer action has no governance semantics: {proposal.action}")
...
def materialize_layer_pheromone_proposals(
    *,
    ...
    if isinstance(current_step, bool) or not isinstance(current_step, int) or current_step < 0:
        raise GovernanceError("layer pheromone proposal step must be a non-negative integer")
    ordered = validate_layer_proposals(proposals, candidate_set=candidate_set, target=target)
    ...
    for proposal in ordered:
        if proposal.action != "propose_pheromone":
            continue
        if layer_action_effect(proposal, policy) != "bounded_pheromone_deposit_proposed":
            continue
        subject_type = proposal.metadata.get("subject_type", "candidate")
        subject_id = proposal.metadata.get("subject_id", proposal.candidate_id)
        if not is_nonblank_string(subject_type) or (
            subject_type not in SUPPORTED_PHEROMONE_SUBJECT_TYPES
            and not is_extension_pheromone_value(subject_type)
        ):
            raise GovernanceError("layer pheromone proposal subject_type is invalid")
        if not is_nonblank_string(subject_id):
            raise GovernanceError("layer pheromone proposal subject_id must be non-blank")
        if neighborhood is not None:
            validate_pheromone_subject_binding(
                neighborhood,
        ...
        if not math.isfinite(strength) or strength <= 0:
            raise GovernanceError("materialized layer pheromone strength must be finite and positive")
        trails.append(
            PheromoneTrail(
                candidate_id=proposal.candidate_id,
                strength=strength,
...
def validate_strategy_bias(
    ...
    *,
    ...
    if bias.layer_id != "evolutionary":
        raise GovernanceError("StrategyBias may only be proposed by the evolutionary layer")
    if not is_nonblank_string(bias.source_id):
        raise GovernanceError("StrategyBias source_id is required")
    if bias.target != target:
        raise GovernanceError(f"StrategyBias targets {bias.target}, not active target {target}")
    ...
    *,
    ...
    for bias in items:
        if bias.trace_event_id in trace_ids:
            raise GovernanceError(f"duplicate StrategyBias trace_event_id: {bias.trace_event_id}")
        ...
        identities.add(identity)
    return tuple(sorted(items, key=lambda item: (item.candidate_id, item.source_id, item.trace_event_id)))


def validate_layer_performance_snapshot(snapshot: LayerPerformanceSnapshot) -> None:
    if not isinstance(snapshot.layer_id, str) or snapshot.layer_id not in SUPPORTED_LAYER_IDS:
...
def validate_layer_performance_snapshots(
    ...
    for snapshot in items:
        ...
        if snapshot.layer_id in seen:
            raise GovernanceError(f"duplicate layer performance snapshot: {snapshot.layer_id}")
...
def assess_layer_confidences(proposals: list[LayerProposal]) -> dict[str, float]:
    ...
    for proposal in sorted(proposals, key=layer_proposal_key):
        if not isinstance(proposal.layer_id, str) or proposal.layer_id not in SUPPORTED_LAYER_IDS:
            raise GovernanceError(f"unsupported layer id: {proposal.layer_id}")
        values[proposal.layer_id].append(_bounded_rate(proposal.confidence, "layer proposal confidence"))
    return {
    ...
    *,
    ...
    for layer_id in sorted(SUPPORTED_LAYER_IDS):
        ...
        if not math.isfinite(weight):
            raise GovernanceError(f"allocated layer weight must remain finite: {layer_id}")
        weights[layer_id] = weight
    return weights


def _strong_preference(proposal: LayerProposal, policy: LayerCoordinationPolicy) -> bool:
    threshold = policy.confidence_thresholds.get(proposal.layer_id, 0.0)
    return (
...
def detect_layer_conflicts(
    ...
    for index, proposal in enumerate(strong):
        for other in strong[index + 1 :]:
            if proposal.layer_id == other.layer_id or proposal.candidate_id == other.candidate_id:
                continue
            if abs(proposal.confidence - other.confidence) <= policy.conflict_threshold:
                conflicts.add("candidate_support_conflict")
    emergencies = [proposal for proposal in ordered if _active_emergency(proposal, policy)]
    if emergencies:
    ...
    snapshot_by_layer = {snapshot.layer_id: snapshot for snapshot in validated_snapshots}
    ...
    *,
    ...
    if conflicts:
        resolvers = [
            ...
            and proposal.confidence >= policy.confidence_thresholds.get("metacognitive", 0.0)
            and proposal.support > proposal.risk
        ]
        if not (set(conflicts) & hard_conflicts) and resolvers:
            resolver = sorted(
                resolvers,
                key=lambda item: (-item.confidence, -item.support, item.candidate_id, item.trace_event_id),
            )[0]
    ...
    positive_scores = {candidate_id: score for candidate_id, score in candidate_scores.items() if score > 0}
    if positive_scores:
        candidate_id = sorted(positive_scores.items(), key=lambda item: (-item[1], item[0]))[0][0]
...
def proposal_score_delta(
    ...
    if proposal.confidence < threshold or is_extension_layer_action(proposal.action):
        return 0.0
    if proposal.action in EMERGENCY_ACTIONS or proposal.proposed_pheromone_kind in {"alarm", "cautionary"}:
        ...
        delta = max(0.0, proposal.support - proposal.risk) * proposal.confidence * weight
    else:
        delta = 0.0
...
def evaluate_layer_coordination(
    *,
    ...
    if not fallback.safe_fallback:
        raise GovernanceError(f"layer fallback candidate is not marked safe: {fallback.id}")
    validate_layer_coordination_policy(policy)
    if not policy.enabled:
        return LayerCoordinationState(resolution="disabled")

    ordered_proposals = validate_layer_proposals(proposals, candidate_set=candidate_set, target=target)
    ...
    if duplicate_lineage:
        raise GovernanceError(f"duplicate layer lineage across proposal and StrategyBias: {sorted(duplicate_lineage)[0]}")

    confidences = assess_layer_confidences(list(ordered_proposals))
    emergency_active = any(_active_emergency(proposal, policy) for proposal in ordered_proposals)
    ...
    candidate_scores = {candidate.id: 0.0 for candidate in active_candidate_set.candidates}
    touched_candidates: set[str] = set()
    for proposal in ordered_proposals:
        if not proposal.candidate_id:
            continue
        delta = proposal_score_delta(proposal, policy, weights.get(proposal.layer_id, 0.0))
        if delta == 0:
            continue
        touched_candidates.add(proposal.candidate_id)
        add_breakdown(breakdown, proposal.candidate_id, f"layer_{proposal.layer_id}", delta)
    ...
    conflicts = detect_layer_conflicts(list(ordered_proposals), policy, list(validated_snapshots))
    ...
    selected, fallback_used, resolution = resolve_layer_conflicts(
        ...
        candidate_scores={candidate_id: candidate_scores[candidate_id] for candidate_id in touched_candidates},
    ...
    return LayerCoordinationState(
        ...
        pheromone_proposal_trace_ids=tuple(
            ...
            for proposal in ordered_proposals
            if proposal.action == "propose_pheromone"
            and action_effects[proposal.trace_event_id] == "bounded_pheromone_deposit_proposed"
        ),
    )
