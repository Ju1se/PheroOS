...
# The canonical event remains provider-neutral.  This table is the versioned
# contract for built-in events whose lineage affects collective authority or
# reconstructs Hybrid Pheromone state.  Extension events deliberately remain
# outside this built-in contract.
EVENT_LINEAGE_CONTRACTS: Mapping[str, frozenset[str]] = MappingProxyType({
    "explore": frozenset({"scout_count"}),
    "scout_report": frozenset(
        {
            "scout_id",
            "candidate_id",
            "evidence_id",
    ...
    # Observation events have three explicit variants validated below:
    # exploration state, exploration floor, and replay rejection.
    "pheromone_observe": frozenset(),
    "pheromone_normalize": frozenset(
        {"candidates", "pre_scores", "post_scores", "response_model", "competition_mode"}
    ),
    "layer_proposal": frozenset(
        {
            "layer_id",
            "source_id",
            "action",
    ...
    "coordination_resolve": frozenset(
        {"conflicts", "resolution", "selected_candidate", "fallback_used", "reason", "proposal_lineage"}
    ),
    "policy_adjustment": frozenset(
        {
            "proposed_values",
            "declared_bounds",
            "result",
    ...
    "candidate_score": frozenset(
        {"scores", "score_breakdown", "scout_diversity", "pheromone_source_diversity"}
    ),
    "consensus_check": frozenset({"quorum_threshold", "min_independent_scouts"}),
    "commit": frozenset({"target", "candidate_id", "decision_reason", "upstream_score_lineage"}),
    "fallback": frozenset({"target", "candidate_id", "decision_reason", "upstream_score_lineage"}),
    "output": frozenset(
        {
            "committed_candidate",
            "evidence_provenance",
...
class InMemoryTraceStore:
    ...
    def __init__(self) -> None:
        self.__records: tuple[TraceRecord, ...] = ()
    ...
    @property
    def _records(self) -> tuple[TraceRecord, ...]:
        """Detached compatibility snapshot of the former private field."""
...
def validate_event_lineage(event: TraceEvent) -> None:
    ...
    missing = sorted(field for field in required if field not in event.lineage)
    if missing:
...
def _validate_pheromone_clip_lineage(event: TraceEvent) -> None:
...
def _validate_deposit_clip_payload(event: TraceEvent, payload: dict[str, Any]) -> None:
...
def _validate_feedback_clip_payload(event: TraceEvent, payload: dict[str, Any]) -> None:
    _require_exact_payload_fields(
        payload,
...
def _validate_diffusion_clip_payload(event: TraceEvent, payload: dict[str, Any]) -> None:
    _require_exact_payload_fields(
        payload,
        {"lifecycle", "input", "effective"},
...
def _validate_pheromone_observation(event: TraceEvent) -> None:
    lineage = event.lineage
    event_type = event.event_type
