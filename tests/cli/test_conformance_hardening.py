...
def test_source_conformance_cli_is_cwd_independent(tmp_path: Path) -> None:
    ...
    assert payload["profile"] == "pheroos-source-v1"
...
def test_invalid_manifest_cli_returns_structured_json_and_nonzero_exit(tmp_path: Path) -> None:
...
def test_installed_console_script_and_public_imports_are_cwd_independent(tmp_path: Path) -> None:
    console_script = Path(sys.executable).parent / "pheroos"
    completed = subprocess.run(
        [str(console_script), "validate", str(ROOT / "examples/toy-protocol/capability.json")],
        cwd=tmp_path,
        check=False,
        capture_output=True,
        text=True,
