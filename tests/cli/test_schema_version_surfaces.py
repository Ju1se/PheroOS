...
from pheroos.kernel import KERNEL_PLAN_VERSION_V2, OSPlan, OSPlanDocument
...
def test_legacy_schema_aliases_stay_pinned_while_v2_ids_are_distinct() -> None:
    ...
    assert schema_for("capability-v2")["$id"].endswith(
...
def test_schema_list_reports_alias_status_version_and_digest(capsys: Any) -> None:
    ...
    assert entries["driver-v2"]["schema_version"] == (
