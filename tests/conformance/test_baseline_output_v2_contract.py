...
MATRIX = ROOT / "pheroos" / "conformance" / "checks" / "baseline_output_v2_contract.py"


def test_reference_store_passes_the_active_baseline_output_v2_matrix() -> None:
    ...
    assert result.name == "baseline_output_v2_contract"
    assert result.ok is True, result.detail
    assert result.detail == ""


def test_independent_store_passes_the_same_baseline_output_v2_matrix() -> None:
...
def test_baseline_output_v2_conformance_version_is_exact() -> None:
    assert (
        GOVERNANCE_BASELINE_OUTPUT_CONFORMANCE_VERSION_V2
...
def test_active_matrix_has_no_skip_or_not_applicable_result_lane() -> None:
    for adapter in (
        ReferenceGovernanceStateStoreConformanceAdapterV2(),
        IndependentStdlibGovernanceStateStoreV2Adapter(),
    ):
...
def test_baseline_output_v2_matrix_rejects_incomplete_and_unknown_adapters() -> None:
    class Incomplete:
        implementation_id = "incomplete-baseline-output-v2"

    incomplete = run_governance_baseline_output_conformance_v2(cast(Any, Incomplete()))
    ...
    unknown = run_governance_baseline_output_conformance_v2(UnknownVersion())
    assert unknown.ok is False
    assert unknown.detail == "adapter_version"


def test_baseline_output_v2_matrix_is_total_at_adapter_boundary() -> None:
    class ExplodingAdapter(IndependentStdlibGovernanceStateStoreV2Adapter):
        implementation_id = "exploding-baseline-output-v2"

        def create_domain_v2(self, scope_ref: str):  # type: ignore[no-untyped-def]
            raise RuntimeError(f"unavailable:{scope_ref}")

    result = run_governance_baseline_output_conformance_v2(ExplodingAdapter())

    assert result.ok is False
    assert result.detail.startswith("adapter_exception:RuntimeError:")


def test_matrix_uses_only_public_composition_contracts_and_no_private_oracle() -> None:
    ...
    assert "_evaluate_decision" not in source
    assert "_evaluate_quorum" not in source
