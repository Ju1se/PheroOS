...
@pytest.mark.parametrize(
    "check", CHECKS, ids=lambda check: check.__module__.rsplit(".", 1)[-1]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "extension-scoring-kind-profile", id="extension-scoring-kind-profile"
        ),
...
def legal_policy_variant(variant: str) -> CapabilityManifest:
    ...
    policy = manifest.protocol.collective_decision_policy
    assert policy is not None
    updates = _legal_policy_updates(variant, policy)
    return replace(
        manifest,
        protocol=replace(
            manifest.protocol,
...
def _legal_policy_updates(
    variant: str,
    policy: CollectiveDecisionPolicy,
) -> dict[str, object]:
    simple = {
        "source-cap-0.001": {"pheromone_per_source_cap": 0.001},
        "source-cap-0.1": {"pheromone_per_source_cap": 0.1},
        "saturation-0": {"pheromone_saturation_threshold": 0.0},
        "saturation-0.001": {"pheromone_saturation_threshold": 0.001},
        "exploration-disabled": {"exploration_enabled": False},
        "quorum-1": {"quorum_threshold": 1},
        "diffusion-hops-3": {"pheromone_diffusion_max_hops": 3},
    }
    if variant in simple:
        return simple[variant]
    if variant.startswith("round-cap-"):
        updates = {
        ...
        return updates
    profile_updates = _profile_variant_updates(variant, policy)
    if profile_updates is not None:
        return profile_updates
    if variant == "all-adjustment-bounds":
        return _all_adjustment_bounds(policy)
    raise AssertionError(f"unknown legal policy variant: {variant}")


def _profile_variant_updates(
    variant: str,
    policy: CollectiveDecisionPolicy,
) -> dict[str, object] | None:
    if variant == "zero-layer-weights":
        return {
            "layer_default_weights": {
                layer_id: 0.0 for layer_id in policy.layer_default_weights
            }
        }
    if variant == "zero-positive-kind-weight":
        return {
    ...
    if variant == "zero-positive-kind-ttl":
        return {
            "pheromone_kind_profiles": {
                **policy.pheromone_kind_profiles,
                "positive": replace(
    ...
    if variant == "positive-kind-competitive":
        return {
            "pheromone_kind_profiles": {
                **policy.pheromone_kind_profiles,
                "positive": replace(
    ...
    if variant == "extension-only-kind-profile":
        return {
            "pheromone_kind_profiles": {
                "x-a.kind": PheromoneKindProfile(weight=1.0),
            }
        }
    if variant == "extension-scoring-kind-profile":
        return {
            "pheromone_kind_profiles": {
    ...
    if variant == "stale-only-kind-profile":
        return {
            "pheromone_kind_profiles": {
...
def _all_adjustment_bounds(policy: CollectiveDecisionPolicy) -> dict[str, object]:
    return {
