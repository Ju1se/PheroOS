...
def test_output_requires_candidate_evidence_stop_resolution_and_permission() -> None:
    ...
    evidence = EvidenceGraph(
        nodes=[EvidenceNode(id="e1", content="Claim", provenance="source")]
    )
    ...
    assert (
        output_authorized(
            ...
            [
                ...
                StopResolution(
                    target="decision:review", action="publish", blocked=False
                ),
            ],
            publication_permission=True,
            candidate_set=candidates(),
...
def test_output_denied_when_evidence_provenance_is_missing() -> None:
    ...
    decision = committed()
    evidence = EvidenceGraph(
        nodes=[EvidenceNode(id="e1", content="Claim", provenance="")]
    )
